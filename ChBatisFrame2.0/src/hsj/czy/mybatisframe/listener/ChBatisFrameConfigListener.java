...
public class ChBatisFrameConfigListener implements ServletContextListener {

    public static Map<String, String> entity_table = new HashMap<String, String>();
    public static char                separator;

    private String                    tableScan;

    public void init() {
        if (tableScan != null) {
            String[] packages = tableScan.split(",");
            initTable(packages);
        }
    }
    ...
    public void contextInitialized(ServletContextEvent arg0) {
        try {
            ...
            File xmlFile = new File(sc.getRealPath("/WEB-INF/classes/ChBatis.xml"));
            SAXReader saxReader = new SAXReader();
            Document document = saxReader.read(xmlFile);
            Element root = document.getRootElement();
            Element table_scan = root.element("table-scan");
            Element s = root.element("separator");
            if (table_scan != null) {
                String[] packages = table_scan.attribute("entity-package").getValue().split(",");
                initTable(packages);
            }
            if (s != null) {
                separator = s.attribute("value").getValue().charAt(0);
            }
    ...
    private void initTable(String[] packages) {
        Table t = null;
        String tname = null;
        Method[] methods = null;
        ...
        for (String string : packages) {
            ...
            for (Class c : clist) {
                ...
                if (t != null) {
                    ...
                    entity_table.put(c.getName(), tname);
    ...
    public static String pojoNameToColumnName(String name) {
        return CamelCaseUtils.toUnderlineName(name);
    }

    public static String columnNameToPojoName(String name) {
        return CamelCaseUtils.toCamelCase(name);
    }

    public String getTableScan() {
        return tableScan;
    }

    public void setTableScan(String tableScan) {
